...
- **content**：应用列表侧栏 + app 详情的 Channels 标签（版本表、下载/检查计数）与 Settings 标签（仅 Release log 分区，用于维护发布日志语言配置）。无 Integration / API keys 标签，无新建 channel 按钮，无新建应用按钮，无设置入口。
- **developer**：content 所见 + Integration 标签 + API keys 标签 + 新建 channel + 新建应用入口（/apps 头部按钮与空态 CTA）+ app Settings 标签（编辑应用配置；删除应用区块仍仅 admin）。无设置入口；不见趋势图、版本统计入口与 release notes 编辑入口（面向内容与运营，工程视角不需要）。
...
- [ ] developer 角色：另有 Integration 与 API keys 标签、新建 channel、新建应用入口（/apps 头部按钮与空态 CTA）及 app Settings 标签（编辑表单可见，删除应用区块不可见）；无设置入口；Channels 标签内不见趋势图、版本统计按钮与 release notes 编辑按钮。
- [ ] admin 角色：全部入口可见。
