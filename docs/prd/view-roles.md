...
- **content**：应用列表侧栏 + app 详情的 Channels 标签（版本表含 draft、下载/检查计数）与 Settings 标签（仅 Release log 分区，用于维护发布日志语言配置）。无 Integration / API docs / API keys，无 promote，无新建 channel 按钮，无新建应用按钮，无设置入口。
- **developer**：content 所见 + Integration 标签 + API docs 标签（ReDoc）+ API keys 标签 + 新建 channel + 新建应用入口（/apps 头部按钮与空态 CTA）+ promote + app Settings 标签（编辑应用配置；删除应用区块仍仅 admin）。无设置入口；不见趋势图、版本统计入口与 release notes 编辑入口（面向内容与运营，工程视角不需要）。
