...
1. **Init** — `POST {baseUrl}/api/v1/upload/init` with header `Authorization: Bearer shk_...` and body:
   ...
   ```
   The file list is every file in the electron-builder output directory (installers, `*.blockmap`, `latest*.yml`) and must include at least one `.yml`. The response contains `uploadId` and, per file, a presigned `uploadUrl`.
...
3. **Finalize** — `POST {baseUrl}/api/v1/upload/finalize` with the same auth header and body `{ "uploadId": "<id>", "app": "<appSlug>" }`. Shukka verifies the uploaded objects, parses the yml metadata, and flips the channel to the new version atomically. A yml whose `version` disagrees with the declared version fails the whole release.

## Steps

1. Build the app with electron-builder; the output directory (usually `dist/`) is what gets published.
2. Read the version from the `version:` field of the `latest*.yml` in the output directory, unless the user gave a version explicitly.
3. Run the publish protocol above against the output directory.
4. Verify: `GET {baseUrl}/api/update/{appSlug}/{channel}/latest.yml` (no auth) now reports the new version. Report the published version and channel to the user.
